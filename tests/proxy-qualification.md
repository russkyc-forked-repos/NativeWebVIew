...
The managed test suite checks proxy resolution, unsupported targets, argument merging, fallback policy, SOCKS5 protocol behavior, long-lived tunnels, and registry ownership/rollback. Fake native stores in registry tests do not establish cookie continuity or WKWebView routing.
...
4. macOS forwarding startup failure, stopped listener, refused outbound connection, and wrong credentials: bounded failure without system fallback for destinations requiring forwarding.
